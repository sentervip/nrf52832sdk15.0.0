...
static uint8_t g_CapFlag = 0;  //0 or 1
static uint8_t g_CapBuf[MAX_SPI_BUF] = {0};
static uint8_t g_save[2000*6*3] = {0};
static uint16_t g_CapCnt = MAX_SPI_BUF;
...
{   
	//高压初始化
	nrf_gpio_cfg_output(HW_HIGH_VOLTAGE_PIN);
	HVOL_SET_L;
	//spi初始化
	nrf_gpio_cfg_output(SPIM0_SS_PIN);		
	SPI_CS_H;
	nrf_gpio_cfg_output(FPGA_READ_CS);
	FPGA_READ_CS_H;
...
int SpiWrite(uint8_t *pData,uint16_t len)
{
	ret_code_t iRet;
	SPI_CS_L;
...
static void nus_data_handler(ble_nus_evt_t * p_evt)
{
	  ...
	  if (p_evt->type == BLE_NUS_EVT_TX_RDY)
		{
			...
			if(g_cnt++ %100 == 0)
				NRF_LOG_INFO("time: %d *10ms == bytes send: %d Bytes == avg speed: %d B/s\n",m_cnt_10ms,m_len_sent,m_len_sent * 100/m_cnt_10ms);	
#endif			
    ...
    {		
#if defined (METHOD1) || defined (METHOD1_PLUS)		  
		    app_timer_start(m_timer_speed, APP_TIMER_TICKS(7),NULL);
#endif
#if defined (METHOD2) || defined (METHOD2_PLUS)
			err_code = ble_nus_data_send(&m_nus, m_data_array, &length, m_conn_handle);
			if ( (err_code != NRF_ERROR_INVALID_STATE) && (err_code != NRF_ERROR_RESOURCES) &&
					 (err_code != NRF_ERROR_NOT_FOUND) )
			{
...
static void ble_evt_handler(ble_evt_t const * p_ble_evt, void * p_context)
{
    ...
    switch (p_ble_evt->header.evt_id)
    {
        ...
        case BLE_GAP_EVT_PHY_UPDATE_REQUEST:
        {
            ...
            {
#ifdef DLE_ON
                .rx_phys = BLE_GAP_PHY_AUTO,    //BLE_GAP_PHY_AUTO
                .tx_phys = BLE_GAP_PHY_1MBPS,    //BLE_GAP_PHY_1MBPS
#else  
        ...
        case BLE_GATTC_EVT_TIMEOUT:
            ...
            APP_ERROR_CHECK(err_code);   HVOL_SET_L;
            break;
...
void gatt_evt_handler(nrf_ble_gatt_t * p_gatt, nrf_ble_gatt_evt_t const * p_evt)
{
    if ((m_conn_handle == p_evt->conn_handle) && (p_evt->evt_id == NRF_BLE_GATT_EVT_ATT_MTU_UPDATED))
    {
        ...
        NRF_LOG_INFO("Data len MTU is set to 0x%X(%d)", m_ble_nus_max_data_len, m_ble_nus_max_data_len);
    }
...
void uart_event_handle(app_uart_evt_t * p_event)
{
    ...
    uint32_t       err_code;
	uint8_t tx[20]={0xa5,0xa5,0xa5,0xa5,0xa5};
...
int main(void)
{
    ...
    for (;;)
    {
		...
		if(g_CapFlag == 2 && g_FPGAReadyFlg){	
			...
			g_CapFlag = 0;	
			g_FPGAReadyFlg = 0;			
			NRF_LOG_INFO("\r\n read spi end,i=%d,mtu=%d!\r\n",i,MAX_SPI2APP_CNT);
		}//end if(g_CapFlag == 2 && g_FPGAReadyFlg){	
